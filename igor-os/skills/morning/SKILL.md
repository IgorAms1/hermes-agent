...
**Before outputting work priorities, check memory exclusions and current task state.** Scan the memory/user-profile blocks already injected into context for entries with "NOT", "don't assign", "not my", "не моя", or "не назначай". If the injected memory is insufficient and file tools are available, read `$HERMES_HOME/memories/MEMORY.md` and `$HERMES_HOME/memories/USER.md`; otherwise use `session_search` with those terms plus known risky entities such as ASBIS or Salesforce. Filter out tasks Igor explicitly flagged as someone else's responsibility.

If the brief surfaces Semaphore / Obsidian-derived work priorities, do **not** choose from memory or last week's summaries. Load/use `read-semaphore-tasks` and read `~/bin/hermes_read_tasks.sh` first, then present only the smallest relevant slice: P0s, blocked items, and one next-action candidate. Keep the distinction explicit: task state comes from `active_tasks.md`; the recommended focus is Hermes's current read. This prevents morning briefs from resurrecting stale partner tasks or assigning work that the task file has already moved to waiting/done.
...
- **Cron-safe scripting / `execute_code` quoting trap with inline Python.** Morning cron may run under approval settings where `execute_code` is unavailable; do not make the brief depend on it. For multi-line Python/Hindsight/calendar helpers in cron, prefer normal tools: `write_file` a standalone `/tmp/*.py`, then run it with `terminal("python3 /tmp/script.py")`. This also avoids the nested quoting trap where `terminal("python3 -c \"complex code\"")` from inside `execute_code` breaks on quotes, backslashes, and multiline strings. Apply this to Hindsight recall, calendar reads, and session file extraction — any multi-line Python that needs `terminal()`. See `evening` skill → `references/session-jsonl-parsing.md` for the session extraction pattern.
  - **Second-order trap:** don't build that `/tmp/*.py` script as a normal nested Python string inside another Python layer unless every backslash is escaped (`'\\n'`, not `'\n'`). Safer options: call the `write_file` tool directly from the main agent, or use a raw triple-quoted string (`r'''...'''`) only when `execute_code` is actually available. This specifically matters for Hindsight recall scripts that contain `.replace('\n', ' ')`; one missed escape turns into an unterminated string in the generated script.
...
- **Memory near capacity.** When memory usage exceeds ~85%, offer Igor a structured prune: present all entries tagged as remove/compact/keep, get approval, batch execute. If Igor explicitly complains that Hermes memory contains Hindsight-shaped detail or tells you to clean it, treat that as approval and prune actively: retain fuller details to Hindsight first, compress/remove Hermes entries, then report before/after percentages. To expand limits instead: `hermes config set memory.memory_char_limit <N>` and `hermes config set memory.user_char_limit <N>`. Full pruning methodology in `igor-os/memory` → `Interactive prune when Igor flags memory bloat` and `references/memory-management.md`. For Hindsight migration (offload durable facts, keep only operational guardrails) see `references/hermes-to-hindsight-migration.md`. Memory architecture convention (pointers in memory, details in files) documented in `references/memory-architecture.md`.
